...
Next, the TextBlob package converts the lyrics strings into a format it can understand and it interprets the language of the song. If the language is not English, our function will not be able to identify the kid_safe count, love and mood count. It will only give back to the length of the non-english song.
...
The Argeparse package ensures a non-empty package is passed to the function to set it rolling.
...
CLASSIFY.PY
This file contains all the code for the actual classification and index calculation for each song for the above mentined dimensions.
